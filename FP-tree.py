...
def FP_tree(database,database_count,now_value=-1):
    ...
    for item in sorted_L1:
        ...
        #開始遞迴
        output_item,output_count=FP_tree(new_database,new_database_count,item[0])
...
import time
tStart = time.time()
item,count=FP_tree(database,database_count)
tEnd = time.time()#計時結束
